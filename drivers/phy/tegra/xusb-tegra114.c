...
struct tegra114_xusb_fuse_calibration {
	u32 hs_curr_level[3];
	u32 hs_iref_cap;
...
struct tegra114_xusb_padctl {
	struct tegra_xusb_padctl base;

	struct tegra114_xusb_fuse_calibration fuse;
};

static inline struct tegra114_xusb_padctl *
to_tegra114_xusb_padctl(struct tegra_xusb_padctl *padctl)
{
	return container_of(padctl, struct tegra114_xusb_padctl, base);
}

static int tegra114_xusb_padctl_enable(struct tegra_xusb_padctl *padctl)
{
	u32 value;
...
static int tegra114_xusb_padctl_disable(struct tegra_xusb_padctl *padctl)
{
	u32 value;
...
static int tegra114_usb3_save_context(struct tegra_xusb_padctl *padctl,
				      unsigned int index)
{
...
static int tegra114_hsic_set_idle(struct tegra_xusb_padctl *padctl,
...
#define TEGRA114_LANE(_name, _offset, _shift, _mask, _type)		\
	{								\
		.name = _name,						\
		.offset = _offset,					\
		.shift = _shift,					\
		.mask = _mask,						\
		.num_funcs = ARRAY_SIZE(tegra114_##_type##_functions),	\
		.funcs = tegra114_##_type##_functions,			\
	}

static const char * const tegra114_usb2_functions[] = {
	"snps",
	"xusb",
	"uart",
};

static const struct tegra_xusb_lane_soc tegra114_usb2_lanes[] = {
	TEGRA114_LANE("usb2-0", 0x004,  0, 0x3, usb2),
	TEGRA114_LANE("usb2-1", 0x004,  2, 0x3, usb2),
	TEGRA114_LANE("usb2-2", 0x004,  4, 0x3, usb2),
};

static struct tegra_xusb_lane *
tegra114_usb2_lane_probe(struct tegra_xusb_pad *pad, struct device_node *np,
			 unsigned int index)
{
...
static void tegra114_usb2_lane_remove(struct tegra_xusb_lane *lane)
{
...
static const struct tegra_xusb_lane_ops tegra114_usb2_lane_ops = {
	.probe = tegra114_usb2_lane_probe,
	.remove = tegra114_usb2_lane_remove,
};

static int tegra114_usb2_phy_init(struct phy *phy)
{
	struct tegra_xusb_lane *lane = phy_get_drvdata(phy);

	return tegra114_xusb_padctl_enable(lane->pad->padctl);
}

static int tegra114_usb2_phy_exit(struct phy *phy)
{
	struct tegra_xusb_lane *lane = phy_get_drvdata(phy);

	return tegra114_xusb_padctl_disable(lane->pad->padctl);
}

static int tegra114_usb2_phy_power_on(struct phy *phy)
{
	struct tegra_xusb_lane *lane = phy_get_drvdata(phy);
	struct tegra_xusb_usb2_lane *usb2 = to_usb2_lane(lane);
	struct tegra_xusb_usb2_pad *pad = to_usb2_pad(lane->pad);
	struct tegra_xusb_padctl *padctl = lane->pad->padctl;
	struct tegra114_xusb_padctl *priv;
	...
	priv = to_tegra114_xusb_padctl(padctl);
...
static int tegra114_usb2_phy_power_off(struct phy *phy)
{
	struct tegra_xusb_lane *lane = phy_get_drvdata(phy);
	struct tegra_xusb_usb2_pad *pad = to_usb2_pad(lane->pad);
	struct tegra_xusb_padctl *padctl = lane->pad->padctl;
...
static const struct phy_ops tegra114_usb2_phy_ops = {
	.init = tegra114_usb2_phy_init,
	.exit = tegra114_usb2_phy_exit,
	.power_on = tegra114_usb2_phy_power_on,
	.power_off = tegra114_usb2_phy_power_off,
	.owner = THIS_MODULE,
};

static struct tegra_xusb_pad *
tegra114_usb2_pad_probe(struct tegra_xusb_padctl *padctl,
			...
			struct device_node *np)
{
	...
	pad = &usb2->base;
	pad->ops = &tegra114_usb2_lane_ops;
	pad->soc = soc;
	...
	err = tegra_xusb_pad_register(pad, &tegra114_usb2_phy_ops);
	if (err < 0)
		goto unregister;
...
static void tegra114_usb2_pad_remove(struct tegra_xusb_pad *pad)
{
...
static const struct tegra_xusb_pad_ops tegra114_usb2_ops = {
	.probe = tegra114_usb2_pad_probe,
	.remove = tegra114_usb2_pad_remove,
};

static const struct tegra_xusb_pad_soc tegra114_usb2_pad = {
	.name = "usb2",
	.num_lanes = ARRAY_SIZE(tegra114_usb2_lanes),
	.lanes = tegra114_usb2_lanes,
	.ops = &tegra114_usb2_ops,
};

static const char * const tegra114_ulpi_functions[] = {
	"snps",
	"xusb",
};

static const struct tegra_xusb_lane_soc tegra114_ulpi_lanes[] = {
	TEGRA114_LANE("ulpi-0", 0x004, 12, 0x1, ulpi),
};

static struct tegra_xusb_lane *
tegra114_ulpi_lane_probe(struct tegra_xusb_pad *pad, struct device_node *np,
			 unsigned int index)
{
...
static void tegra114_ulpi_lane_remove(struct tegra_xusb_lane *lane)
{
...
static const struct tegra_xusb_lane_ops tegra114_ulpi_lane_ops = {
	.probe = tegra114_ulpi_lane_probe,
	.remove = tegra114_ulpi_lane_remove,
};

static int tegra114_ulpi_phy_init(struct phy *phy)
{
	struct tegra_xusb_lane *lane = phy_get_drvdata(phy);
...
static int tegra114_ulpi_phy_exit(struct phy *phy)
{
	struct tegra_xusb_lane *lane = phy_get_drvdata(phy);
...
static int tegra114_ulpi_phy_power_on(struct phy *phy)
{
	return 0;
}

static int tegra114_ulpi_phy_power_off(struct phy *phy)
{
	return 0;
}

static const struct phy_ops tegra114_ulpi_phy_ops = {
	.init = tegra114_ulpi_phy_init,
	.exit = tegra114_ulpi_phy_exit,
	.power_on = tegra114_ulpi_phy_power_on,
	.power_off = tegra114_ulpi_phy_power_off,
	.owner = THIS_MODULE,
};

static struct tegra_xusb_pad *
tegra114_ulpi_pad_probe(struct tegra_xusb_padctl *padctl,
			const struct tegra_xusb_pad_soc *soc,
			struct device_node *np)
{
	...
	pad = &ulpi->base;
	pad->ops = &tegra114_ulpi_lane_ops;
	pad->soc = soc;
	...
	err = tegra_xusb_pad_register(pad, &tegra114_ulpi_phy_ops);
	if (err < 0)
		goto unregister;
...
static void tegra114_ulpi_pad_remove(struct tegra_xusb_pad *pad)
{
...
static const struct tegra_xusb_pad_ops tegra114_ulpi_ops = {
	.probe = tegra114_ulpi_pad_probe,
	.remove = tegra114_ulpi_pad_remove,
};

static const struct tegra_xusb_pad_soc tegra114_ulpi_pad = {
	.name = "ulpi",
	.num_lanes = ARRAY_SIZE(tegra114_ulpi_lanes),
	.lanes = tegra114_ulpi_lanes,
	.ops = &tegra114_ulpi_ops,
};

static const char * const tegra114_hsic_functions[] = {
	"snps",
	"xusb",
};

static const struct tegra_xusb_lane_soc tegra114_hsic_lanes[] = {
	TEGRA114_LANE("hsic-0", 0x004, 14, 0x1, hsic),
	TEGRA114_LANE("hsic-1", 0x004, 15, 0x1, hsic),
};

static struct tegra_xusb_lane *
tegra114_hsic_lane_probe(struct tegra_xusb_pad *pad, struct device_node *np,
			 unsigned int index)
{
...
static void tegra114_hsic_lane_remove(struct tegra_xusb_lane *lane)
{
...
static const struct tegra_xusb_lane_ops tegra114_hsic_lane_ops = {
	.probe = tegra114_hsic_lane_probe,
	.remove = tegra114_hsic_lane_remove,
};

static int tegra114_hsic_phy_init(struct phy *phy)
{
	struct tegra_xusb_lane *lane = phy_get_drvdata(phy);
...
static int tegra114_hsic_phy_exit(struct phy *phy)
{
	struct tegra_xusb_lane *lane = phy_get_drvdata(phy);
...
static int tegra114_hsic_phy_power_on(struct phy *phy)
{
	struct tegra_xusb_lane *lane = phy_get_drvdata(phy);
	struct tegra_xusb_hsic_lane *hsic = to_hsic_lane(lane);
...
static int tegra114_hsic_phy_power_off(struct phy *phy)
{
	struct tegra_xusb_lane *lane = phy_get_drvdata(phy);
	struct tegra_xusb_hsic_pad *pad = to_hsic_pad(lane->pad);
	struct tegra_xusb_padctl *padctl = lane->pad->padctl;
...
static const struct phy_ops tegra114_hsic_phy_ops = {
	.init = tegra114_hsic_phy_init,
	.exit = tegra114_hsic_phy_exit,
	.power_on = tegra114_hsic_phy_power_on,
	.power_off = tegra114_hsic_phy_power_off,
	.owner = THIS_MODULE,
};

static struct tegra_xusb_pad *
tegra114_hsic_pad_probe(struct tegra_xusb_padctl *padctl,
			const struct tegra_xusb_pad_soc *soc,
			struct device_node *np)
{
	...
	pad = &hsic->base;
	pad->ops = &tegra114_hsic_lane_ops;
	pad->soc = soc;
	...
	err = tegra_xusb_pad_register(pad, &tegra114_hsic_phy_ops);
	if (err < 0)
		goto unregister;
...
static void tegra114_hsic_pad_remove(struct tegra_xusb_pad *pad)
{
...
static const struct tegra_xusb_pad_ops tegra114_hsic_ops = {
	.probe = tegra114_hsic_pad_probe,
	.remove = tegra114_hsic_pad_remove,
};

static const struct tegra_xusb_pad_soc tegra114_hsic_pad = {
	.name = "hsic",
	.num_lanes = ARRAY_SIZE(tegra114_hsic_lanes),
	.lanes = tegra114_hsic_lanes,
	.ops = &tegra114_hsic_ops,
};

static const char * const tegra114_pcie_functions[] = {
	"pcie",
	"usb3-ss",
	"sata",
};

static const struct tegra_xusb_lane_soc tegra114_pcie_lanes[] = {
	TEGRA114_LANE("pcie-0", 0x134, 16, 0x3, pcie),
	TEGRA114_LANE("pcie-1", 0x134, 18, 0x3, pcie),
	TEGRA114_LANE("pcie-2", 0x134, 20, 0x3, pcie),
	TEGRA114_LANE("pcie-3", 0x134, 22, 0x3, pcie),
	TEGRA114_LANE("pcie-4", 0x134, 24, 0x3, pcie),
};

static struct tegra_xusb_lane *
tegra114_pcie_lane_probe(struct tegra_xusb_pad *pad, struct device_node *np,
			 unsigned int index)
{
...
static void tegra114_pcie_lane_remove(struct tegra_xusb_lane *lane)
{
...
static const struct tegra_xusb_lane_ops tegra114_pcie_lane_ops = {
	.probe = tegra114_pcie_lane_probe,
	.remove = tegra114_pcie_lane_remove,
};

static int tegra114_pcie_phy_init(struct phy *phy)
{
	struct tegra_xusb_lane *lane = phy_get_drvdata(phy);
...
static int tegra114_pcie_phy_exit(struct phy *phy)
{
	struct tegra_xusb_lane *lane = phy_get_drvdata(phy);
...
static int tegra114_pcie_phy_power_on(struct phy *phy)
{
	struct tegra_xusb_lane *lane = phy_get_drvdata(phy);
	struct tegra_xusb_padctl *padctl = lane->pad->padctl;
...
static int tegra114_pcie_phy_power_off(struct phy *phy)
{
	struct tegra_xusb_lane *lane = phy_get_drvdata(phy);
	struct tegra_xusb_padctl *padctl = lane->pad->padctl;
	u32 value;
...
static const struct phy_ops tegra114_pcie_phy_ops = {
	.init = tegra114_pcie_phy_init,
	.exit = tegra114_pcie_phy_exit,
	.power_on = tegra114_pcie_phy_power_on,
	.power_off = tegra114_pcie_phy_power_off,
	.owner = THIS_MODULE,
};

static struct tegra_xusb_pad *
tegra114_pcie_pad_probe(struct tegra_xusb_padctl *padctl,
			const struct tegra_xusb_pad_soc *soc,
			struct device_node *np)
{
	...
	pad = &pcie->base;
	pad->ops = &tegra114_pcie_lane_ops;
	pad->soc = soc;
	...
	err = tegra_xusb_pad_register(pad, &tegra114_pcie_phy_ops);
	if (err < 0)
		goto unregister;
...
static void tegra114_pcie_pad_remove(struct tegra_xusb_pad *pad)
{
...
static const struct tegra_xusb_pad_ops tegra114_pcie_ops = {
	.probe = tegra114_pcie_pad_probe,
	.remove = tegra114_pcie_pad_remove,
};

static const struct tegra_xusb_pad_soc tegra114_pcie_pad = {
	.name = "pcie",
	.num_lanes = ARRAY_SIZE(tegra114_pcie_lanes),
	.lanes = tegra114_pcie_lanes,
	.ops = &tegra114_pcie_ops,
};

static const struct tegra_xusb_lane_soc tegra114_sata_lanes[] = {
	TEGRA114_LANE("sata-0", 0x134, 26, 0x3, pcie),
};

static struct tegra_xusb_lane *
tegra114_sata_lane_probe(struct tegra_xusb_pad *pad, struct device_node *np,
			 unsigned int index)
{
...
static void tegra114_sata_lane_remove(struct tegra_xusb_lane *lane)
{
...
static const struct tegra_xusb_lane_ops tegra114_sata_lane_ops = {
	.probe = tegra114_sata_lane_probe,
	.remove = tegra114_sata_lane_remove,
};

static int tegra114_sata_phy_init(struct phy *phy)
{
	struct tegra_xusb_lane *lane = phy_get_drvdata(phy);
...
static int tegra114_sata_phy_exit(struct phy *phy)
{
	struct tegra_xusb_lane *lane = phy_get_drvdata(phy);
...
static int tegra114_sata_phy_power_on(struct phy *phy)
{
	struct tegra_xusb_lane *lane = phy_get_drvdata(phy);
	struct tegra_xusb_padctl *padctl = lane->pad->padctl;
	unsigned long timeout;
...
static int tegra114_sata_phy_power_off(struct phy *phy)
{
	struct tegra_xusb_lane *lane = phy_get_drvdata(phy);
	struct tegra_xusb_padctl *padctl = lane->pad->padctl;
	u32 value;
...
static const struct phy_ops tegra114_sata_phy_ops = {
	.init = tegra114_sata_phy_init,
	.exit = tegra114_sata_phy_exit,
	.power_on = tegra114_sata_phy_power_on,
	.power_off = tegra114_sata_phy_power_off,
	.owner = THIS_MODULE,
};

static struct tegra_xusb_pad *
tegra114_sata_pad_probe(struct tegra_xusb_padctl *padctl,
			const struct tegra_xusb_pad_soc *soc,
			struct device_node *np)
{
	...
	pad = &sata->base;
	pad->ops = &tegra114_sata_lane_ops;
	pad->soc = soc;
	...
	err = tegra_xusb_pad_register(pad, &tegra114_sata_phy_ops);
	if (err < 0)
		goto unregister;
...
static void tegra114_sata_pad_remove(struct tegra_xusb_pad *pad)
{
...
static const struct tegra_xusb_pad_ops tegra114_sata_ops = {
	.probe = tegra114_sata_pad_probe,
	.remove = tegra114_sata_pad_remove,
};

static const struct tegra_xusb_pad_soc tegra114_sata_pad = {
	.name = "sata",
	.num_lanes = ARRAY_SIZE(tegra114_sata_lanes),
	.lanes = tegra114_sata_lanes,
	.ops = &tegra114_sata_ops,
};

static const struct tegra_xusb_pad_soc *tegra114_pads[] = {
	&tegra114_usb2_pad,
	&tegra114_ulpi_pad,
	&tegra114_hsic_pad,
	&tegra114_pcie_pad,
...
static int tegra114_usb2_port_enable(struct tegra_xusb_port *port)
{
	return 0;
}

static void tegra114_usb2_port_disable(struct tegra_xusb_port *port)
{
}

static struct tegra_xusb_lane *
tegra114_usb2_port_map(struct tegra_xusb_port *port)
{
	return tegra_xusb_find_lane(port->padctl, "usb2", port->index);
}

static const struct tegra_xusb_port_ops tegra114_usb2_port_ops = {
	.release = tegra_xusb_usb2_port_release,
	.remove = tegra_xusb_usb2_port_remove,
	.enable = tegra114_usb2_port_enable,
	.disable = tegra114_usb2_port_disable,
	.map = tegra114_usb2_port_map,
};

static int tegra114_ulpi_port_enable(struct tegra_xusb_port *port)
{
	return 0;
}

static void tegra114_ulpi_port_disable(struct tegra_xusb_port *port)
{
}

static struct tegra_xusb_lane *
tegra114_ulpi_port_map(struct tegra_xusb_port *port)
{
	return tegra_xusb_find_lane(port->padctl, "ulpi", port->index);
}

static const struct tegra_xusb_port_ops tegra114_ulpi_port_ops = {
	.release = tegra_xusb_ulpi_port_release,
	.enable = tegra114_ulpi_port_enable,
	.disable = tegra114_ulpi_port_disable,
	.map = tegra114_ulpi_port_map,
};

static int tegra114_hsic_port_enable(struct tegra_xusb_port *port)
{
	return 0;
}

static void tegra114_hsic_port_disable(struct tegra_xusb_port *port)
{
}

static struct tegra_xusb_lane *
tegra114_hsic_port_map(struct tegra_xusb_port *port)
{
	return tegra_xusb_find_lane(port->padctl, "hsic", port->index);
}

static const struct tegra_xusb_port_ops tegra114_hsic_port_ops = {
	.release = tegra_xusb_hsic_port_release,
	.enable = tegra114_hsic_port_enable,
	.disable = tegra114_hsic_port_disable,
	.map = tegra114_hsic_port_map,
};

static int tegra114_usb3_port_enable(struct tegra_xusb_port *port)
{
...
static void tegra114_usb3_port_disable(struct tegra_xusb_port *port)
{
	struct tegra_xusb_padctl *padctl = port->padctl;
	u32 value;
...
static const struct tegra_xusb_lane_map tegra114_usb3_map[] = {
	{ 0, "pcie", 0 },
	{ 1, "pcie", 1 },
	{ 1, "sata", 0 },
	{ 0, NULL,   0 },
...
static struct tegra_xusb_lane *
tegra114_usb3_port_map(struct tegra_xusb_port *port)
{
	return tegra_xusb_port_find_lane(port, tegra114_usb3_map, "usb3-ss");
}

static const struct tegra_xusb_port_ops tegra114_usb3_port_ops = {
	.release = tegra_xusb_usb3_port_release,
	.enable = tegra114_usb3_port_enable,
	.disable = tegra114_usb3_port_disable,
	.map = tegra114_usb3_port_map,
};

static int
tegra114_xusb_read_fuse_calibration(struct tegra114_xusb_fuse_calibration *fuse)
{
	unsigned int i;
	int err;
	u32 value;
...
tegra114_xusb_padctl_probe(struct device *dev,
			   const struct tegra_xusb_padctl_soc *soc)
{
	struct tegra114_xusb_padctl *padctl;
	int err;
	...
	err = tegra114_xusb_read_fuse_calibration(&padctl->fuse);
	if (err < 0)
		return ERR_PTR(err);
...
static void tegra114_xusb_padctl_remove(struct tegra_xusb_padctl *padctl)
{
}

static const struct tegra_xusb_padctl_ops tegra114_xusb_padctl_ops = {
	.probe = tegra114_xusb_padctl_probe,
	.remove = tegra114_xusb_padctl_remove,
	.usb3_save_context = tegra114_usb3_save_context,
	.hsic_set_idle = tegra114_hsic_set_idle,
};

static const char * const tegra114_xusb_padctl_supply_names[] = {
	"avdd-pll-utmip",
	"avdd-pll-erefe",
	"avdd-pex-pll",
	"hvdd-pex-pll-e",
...
const struct tegra_xusb_padctl_soc tegra114_xusb_padctl_soc = {
	.num_pads = ARRAY_SIZE(tegra114_pads),
	.pads = tegra114_pads,
	.ports = {
		.usb2 = {
			.ops = &tegra114_usb2_port_ops,
			.count = 3,
		},
		.ulpi = {
			.ops = &tegra114_ulpi_port_ops,
			.count = 1,
		},
		.hsic = {
			.ops = &tegra114_hsic_port_ops,
			.count = 2,
		},
		.usb3 = {
			.ops = &tegra114_usb3_port_ops,
			.count = 2,
		},
	},
	.ops = &tegra114_xusb_padctl_ops,
	.supply_names = tegra114_xusb_padctl_supply_names,
	.num_supplies = ARRAY_SIZE(tegra114_xusb_padctl_supply_names),
};
EXPORT_SYMBOL_GPL(tegra114_xusb_padctl_soc);

MODULE_AUTHOR("Thierry Reding <treding@nvidia.com>");
MODULE_DESCRIPTION("NVIDIA Tegra 114 XUSB Pad Controller driver");
