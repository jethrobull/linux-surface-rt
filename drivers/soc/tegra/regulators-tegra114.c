...
static int tegra114_core_limit(struct tegra_regulator_coupler *tegra,
			      struct regulator_dev *core_rdev)
{
	int core_min_uV = 0;
	int core_max_uV;
	...
	core_max_uV = max(core_cur_uV, 1400000);
...
static int tegra114_core_cpu_limit(int cpu_uV)
{
	if (cpu_uV < 800000)
		return 950000;
...
static int tegra114_cpu_nominal_uV(void)
{
	switch (tegra_sku_info.cpu_speedo_id) {
	case 10 ... 11:
		return  1100000;
...
static int tegra114_core_nominal_uV(void)
{
...
static int tegra114_voltage_update(struct tegra_regulator_coupler *tegra,
				  struct regulator_dev *cpu_rdev,
				  struct regulator_dev *core_rdev)
{
	 ...
	 */
	core_min_uV = tegra114_core_limit(tegra, core_rdev);
	if (core_min_uV < 0)
		return core_min_uV;
	...
	if (tegra->sys_suspend_mode)
		core_min_uV = clamp(tegra114_core_nominal_uV(),
	 ...
	 * at first.
	 */
	core_min_limited_uV = tegra114_core_cpu_limit(cpu_uV);
	if (core_min_limited_uV < 0)
		return core_min_limited_uV;

	core_min_uV = max(core_min_uV, tegra114_core_cpu_limit(cpu_min_uV));
	...
	/* prepare voltage level for suspend */
	if (tegra->sys_suspend_mode)
		cpu_min_uV = clamp(tegra114_cpu_nominal_uV(),
	...
	pr_info("tegra114 regulator coupler cpu uV %u\n",cpu_uV);
	pr_info("tegra114 regulator coupler cpu min uV %u\n",cpu_min_uV);
	pr_info("tegra114 regulator coupler core uV %u\n",core_uV);
	pr_info("tegra114 regulator coupler core min uV %u\n",core_min_uV);
	while (cpu_uV != cpu_min_uV || core_uV != core_min_uV) {
		...
		err = regulator_set_voltage_rdev(cpu_rdev,
						 ...
						 PM_SUSPEND_ON);
		if (err)
			pr_info("tegra114 regulator coupler could not set cpu regulator %d\n",err);
			return err;
...
update_core:
		...
		err = regulator_set_voltage_rdev(core_rdev,
						 ...
						 PM_SUSPEND_ON);
		if (err)
			pr_info("tegra114 regulator coupler could not set core regulator %d\n",err);
			return err;
...
static int tegra114_regulator_balance_voltage(struct regulator_coupler *coupler,
					     ...
					     suspend_state_t state)
{
	...
	return tegra114_voltage_update(tegra, cpu_rdev, core_rdev);
}

static int tegra114_regulator_prepare_suspend(struct tegra_regulator_coupler *tegra,
					     bool sys_suspend_mode)
{
	int err;
...
static int tegra114_regulator_suspend(struct notifier_block *notifier,
				     unsigned long mode, void *arg)
{
	...
	case PM_SUSPEND_PREPARE:
		ret = tegra114_regulator_prepare_suspend(tegra, true);
		break;

	case PM_POST_HIBERNATION:
	case PM_POST_RESTORE:
	case PM_POST_SUSPEND:
		ret = tegra114_regulator_prepare_suspend(tegra, false);
		break;
	}
...
static int tegra114_regulator_prepare_reboot(struct tegra_regulator_coupler *tegra,
					    bool sys_reboot_mode)
{
	int err;
...
static int tegra114_regulator_reboot(struct notifier_block *notifier,
				    unsigned long event, void *cmd)
{
	...
	ret = tegra114_regulator_prepare_reboot(tegra, true);

	return notifier_from_errno(ret);
}

static int tegra114_regulator_attach(struct regulator_coupler *coupler,
				    struct regulator_dev *rdev)
{
	struct tegra_regulator_coupler *tegra = to_tegra_coupler(coupler);
	...
	if (of_property_read_bool(np, "nvidia,tegra-core-regulator") &&
		...
		pr_info("tegra114 regulator coupler core regulator attached\n");
		return 0;
	}
	...
	if (of_property_read_bool(np, "nvidia,tegra-cpu-regulator") &&
	    !tegra->cpu_rdev) {
		tegra->cpu_rdev = rdev;
		pr_info("tegra114 regulator coupler cpu regulator attached\n");
		return 0;
	}
...
static int tegra114_regulator_detach(struct regulator_coupler *coupler,
				    struct regulator_dev *rdev)
{
	struct tegra_regulator_coupler *tegra = to_tegra_coupler(coupler);
...
static struct tegra_regulator_coupler tegra114_coupler = {
	.coupler = {
		.attach_regulator = tegra114_regulator_attach,
		.detach_regulator = tegra114_regulator_detach,
		.balance_voltage = tegra114_regulator_balance_voltage,
	},
	.reboot_notifier.notifier_call = tegra114_regulator_reboot,
	.suspend_notifier.notifier_call = tegra114_regulator_suspend,
};
...
static int __init tegra_regulator_coupler_init(void)
{
	...
	err = register_reboot_notifier(&tegra114_coupler.reboot_notifier);
	WARN_ON(err);

	err = register_pm_notifier(&tegra114_coupler.suspend_notifier);
	WARN_ON(err);

	pr_info("tegra114 regulator coupler init\n");

	return regulator_coupler_register(&tegra114_coupler.coupler);
}
